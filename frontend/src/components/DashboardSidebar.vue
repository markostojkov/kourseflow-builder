<template>
	<div
		...
		:class="builderStore.showDashboardSidebar ? 'w-56' : 'w-14'">
		<div
			class="flex flex-col overflow-hidden"
			:class="builderStore.showDashboardSidebar ? '' : 'items-center'">
			<div class="mb-2">
				<Dropdown
					:options="[
						{
					...
					<template v-slot="{ open }">
						<div
							class="mx-2 my-0.5 flex items-center justify-between rounded p-2"
							:class="{
								'bg-surface-modal shadow-sm dark:bg-surface-gray-2':
									open && builderStore.showDashboardSidebar,
								'hover:bg-surface-gray-2': builderStore.showDashboardSidebar,
							}">
							<div
			...
			<div class="flex flex-col" v-if="builderProjectFolder.data">
				<Dialog
				...
				<span
					class="mx-2 my-0.5 flex cursor-pointer gap-2 rounded p-2 text-base text-ink-gray-6"
			...
			<div v-if="builderProjectFolder.data?.length" class="mt-4">
				<div class="flex flex-col transition-all duration-300 ease-in-out">
					<span
						class="mx-2 my-0.5 flex h-8 w-full cursor-pointer items-center justify-between gap-2 rounded p-2 py-1 text-base text-ink-gray-6"
						v-show="builderStore.showDashboardSidebar"
		...
		<div class="m-2 flex flex-col gap-1">
			<div
				...
				:class="builderStore.showDashboardSidebar ? 'flex-row' : 'flex-col space-y-3'">
				<div
					class="flex flex-1 items-center"
					:class="builderStore.showDashboardSidebar ? 'flex-row space-x-3' : 'flex-col space-y-3'">
					<Tooltip v-if="!builderStore.showDashboardSidebar"></Tooltip>
				</div>
				<Tooltip :text="builderStore.showDashboardSidebar ? 'Collapse' : 'Expand'">
					<CollapseSidebar
						class="stroke-1.5 size-4 cursor-pointer text-ink-gray-7 duration-300 ease-in-out"
						:class="{
							'[transform:rotateY(180deg)]': !builderStore.showDashboardSidebar,
						}"
